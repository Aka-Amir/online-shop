...
interface ChartGradiant {
  body: CanvasGradient;
  border: CanvasGradient;
}
...
export class StatisticsComponent implements OnInit, AfterViewInit {
  ...
  constructor(private SS: StatisticsService) {
    let incomeChartData = this.SS.getIncomeStatistics("");
    this.incomeLabels = incomeChartData.labels;
    this.incomeData = incomeChartData.data;
}

  gradiantsTheme = {
    bluePurple: {
      body: this.gradiantGenerator(
        'rgba(105, 56, 161, 0.8)',
        'rgba(101, 104, 252, 0.8)'
      ),
      border: this.gradiantGenerator(
        'rgba(56, 58, 161 , 1)',
        'rgba(101, 104, 252, 1)'
      ),
    },
    ...
    GOB: {
      body: this.gradiantGenerator(
        'rgba(194, 229, 156 , 0.8)',
        'rgba(100, 179, 244 , 0.8)'
      ),
  ...
  ngAfterViewInit(): void {
    const chart = this.createChart(
      document.getElementById('sell-chart-linear') as HTMLCanvasElement,
      this.gradiantsTheme.timber,
      this.incomeLabels,
      this.incomeData
    );
  }
